/**
 * InteriorQuoteEditor — Per-room configurator for int-static items on /tarifs.
 ...
 *
 * Used on: QuoteItemCard for int-static items (/tarifs page).
 */
"use client";
...
export function InteriorQuoteEditor({
  ...
  const calc = calcInteriorTotal(floors, pricing);
    ...
    <div className="space-y-2 rounded-xl bg-foreground/5 px-4 py-3 text-xs">
      ...
      <div className="flex items-baseline justify-between gap-2 text-muted-foreground">
        ...
        <span className="flex-shrink-0 font-semibold tabular-nums">
          -{formatPublicPrice(savings, displayCurrency, pricingSettings)}
        </span>
      </div>
