...
export async function POST(request: Request) {
  ...
  if (!validDraftId(safeDraftId)) {
    return NextResponse.json(
      { error: "Invalid application draft" },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    size <= 0 ||
    size > JOB_APPLICATION_MAX_FILE_BYTES
  ) {
    return NextResponse.json(
  ...
  if (!isAllowedJobApplicationMimeType(type)) {
    return NextResponse.json(
      { error: "Allowed file types are PDF, DOC, DOCX, ZIP, JPG, PNG and WebP." },
      { status: 400 },
    );
  }

  const sanitized = String(fileName).replace(/[^a-zA-Z0-9._-]/g, "_");
  const storagePath = `jobs/${safeDraftId}/${crypto.randomUUID()}-${sanitized}`;
